...
from nanobot.config.schema import Config, DebugConfig
...
class ChannelManager:
    ...
    def __init__(
        self, config: Config, bus: MessageBus,
    ...
    def _init_channels(self) -> None:
        ...
        # Raven channel (outbound-only, delivers directly via Raven API)
        if self.config.channels.raven.enabled:
            try:
                from nanobot.channels.raven import RavenChannel
                self.channels["raven"] = RavenChannel(
                    self.config.channels.raven, self.bus
                )
